...
function ajouter_produit($libelle, $prix,$chemin)
{
    ...
    try {
        ...
        $r = $cnx->prepare("insert into produit(libelle,prix,chemin) values(?,?,?)");

        // on execute cette requete
        $r->execute([$libelle, $prix,$chemin]);
    } catch (\Throwable $th) {
...
//fonction pour uploader un fichier et nius retourner le chemin
function uploader($info)  {
    
// var_dump($info['tmp_name']);
$tmp=$info['tmp_name'];
$extension=pathinfo($info['name'],PATHINFO_EXTENSION);
// var_dump($extension);
$autorise=['jpeg','jpg','png','gif'];
if (!in_array($extension,$autorise)) {
 echo "type de fichier non autorise";
 return null;

}
if (filesize($tmp)>100000) {
 echo "cette image doit avoir une taille < 1Mo ";
 return null;
}
$name=md5(uniqid()).".$extension";
// var_dump($name);
  $to="images/$name";
//uploader le fichier dans le serveur 
